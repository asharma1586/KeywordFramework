...
public class ReadReusable {
	...
	for(int i=range_value1;i<=range_value2;i++){
		...
		String orData[]= ExcelUtilities.getORData(Constants.repositoryPath,rPageName,rLocatorName);
			...
			/*String[] orData= ExcelUtilities.getORData(Constants.repositoryPath,rPageName,rLocatorName);
			if (orData[0].equals(null))
			{
				System.out.println("No Need of OR Values");
			}
			else
			{
			orlocatorType=orData[0];
		...
		*/
			//Performing action
		Action_Keywords keywords=new Action_Keywords(rActionKeyword,orlocatorType,orlocatorValue,rData,"1","TestReusable", debugLog, resultLog);
